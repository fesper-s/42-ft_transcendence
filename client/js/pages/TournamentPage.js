    ...
    fetching(`https://${window.ft_transcendence_host}/tournament/`).then((data) => {
      if (!data.current_tournament || data.current_tournament.status === "FN") {
        ...
        if (data.tournaments) {
          for (const tournament of data.tournaments) {
            ...
            tournament_card.setAttribute("size", tournament.gamers_count);
      ...
      if (data.current_tournament) {
        ...
        if (data.current_tournament.status === "PN") {
          ...
          for (const gamer of data.gamers) {
            const player_elem = document.createElement("tournament-player-card");
            player_elem.setAttribute("alias-name", gamer.alias_name);
            player_elem.setAttribute("avatar", gamer.avatar);
          ...
          for (let i = 0; i < 7; ++i) {
            ...
            if (match) {
              ...
              for (let j = 0; j < 2; ++j) {
                const player = match.gamers[j];
                const player_elem = match_elem.querySelector(`.player${j + 1}`);
                player_elem.querySelector("h6").textContent = player.gamer.tournament_name;
                player_elem.querySelector("img").src = player.gamer.avatar;
                if (match.state === "PLY")
