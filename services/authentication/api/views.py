...
from .service import decode_google_id_token, generate_jwt, check_2fa_code, jwt_cookie_required, get_2fa_qr_code, create_gamer
...
def intra_callback_auth(request):
    ...
    gamer_data = {
        ...
        "name": f"{user_response.json()['first_name']} {user_response.json()['last_name']}",
        "avatar": user_response.json()['image']['link'],
    }
    gamer = create_gamer(gamer_data)
    if gamer is None:
        return redirect(f"https://{settings.FT_TRANSCENDENCE_HOST}/login/", permanent=True)
    jwt_token = generate_jwt(gamer.id, gamer.two_factor)
    response = redirect(f"https://{settings.FT_TRANSCENDENCE_HOST}/{'twofa' if gamer.two_factor else 'home'}/", permanent=True)
